...
def _init():
    ...
    if _plugin is None:
        try:
            ...
            import warnings
            warnings.warn(f'Failed to compile CUDA extension for filtered_lrelu. Falling back to CPU implementation. Error: {e}')
...
def _filtered_lrelu_cuda(up=1, down=1, padding=0, gain=np.sqrt(2), slope=0.2, clamp=None, flip_filter=False):
    ...
    class FilteredLReluCuda(torch.autograd.Function):
        ...
        def forward(ctx, x, fu, fd, b, si, sx, sy): # pylint: disable=arguments-differ
            ...
            # Call C++/Cuda plugin if datatype is supported and plugin is available.
            if _plugin is not False and x.dtype in [torch.float16, torch.float32]:
