...
class DragWidget:
    def __init__(self, viz):
        ...
        self.r2             = 12
        # 新增：硬约束背景保护的超参数
        self.use_hard_constraint = False  # 是否使用硬约束
        self.mask_blur_sigma = 3.0  # Mask模糊强度
        self.blend_res_64 = True  # 是否在64x64层融合
        self.blend_res_128 = True  # 是否在128x128层融合
        self.blend_res_256 = True  # 是否在256x256层融合
        #
        self.is_raft           = False
        #
    ...
    def __call__(self, show=True):
        ...
        if show:
            with imgui_utils.grayed_out(self.disabled_time != 0):
                ...
                imgui.text(' ')
                imgui.same_line(viz.label_w)
                _clicked, self.use_hard_constraint = imgui.checkbox('Feature Blending', self.use_hard_constraint)
                
                if self.use_hard_constraint:
                    imgui.text(' ')
                    imgui.same_line(viz.label_w)
                    with imgui_utils.item_width(viz.font_size * 8):
                        changed, self.mask_blur_sigma = imgui.input_float('Blur Sigma', self.mask_blur_sigma, 0.1, 1.0, '%.1f')
                    
                    imgui.text('Blend Layers:')
                    imgui.same_line(viz.label_w)
                    _clicked, self.blend_res_64 = imgui.checkbox('64x64', self.blend_res_64)
                    imgui.same_line()
                    _clicked, self.blend_res_128 = imgui.checkbox('128x128', self.blend_res_128)
                    imgui.same_line()
                    _clicked, self.blend_res_256 = imgui.checkbox('256x256', self.blend_res_256)
                # ============================================
                
                # ========== 新增：RAFT点跟踪选项 ==========
                imgui.text(' ')
                imgui.same_line(viz.label_w)
                _clicked, self.is_raft = imgui.checkbox('Use raft', self.is_raft)
                # ============================================
        ...
        # 新增：硬约束相关参数
        viz.args.use_hard_constraint = self.use_hard_constraint
        viz.args.mask_blur_sigma = self.mask_blur_sigma
        blend_resolutions = []
        if self.blend_res_64:
            blend_resolutions.append(64)
        if self.blend_res_128:
            blend_resolutions.append(128)
        if self.blend_res_256:
            blend_resolutions.append(256)
        if not blend_resolutions:
            blend_resolutions = [64, 128, 256]  # 默认值
        viz.args.blend_resolutions = blend_resolutions
        # 新增：RAFT相关参数
        viz.args.is_raft = self.is_raft
